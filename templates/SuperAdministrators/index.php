 ...
 * @var iterable<\App\Model\Entity\SuperAdministrator> $superAdministrators
 */
...
<div class="superAdministrators index content">
    ...
    <button type="button" class="btn btn-info float-right" onClick="open_add();"> Nuevo Super Administrador</button>
    <br>
    <div class="title">Super Administradores</div>
    <br>
    <div class="table-responsive">
        <table align="center" id="SuperAdministrators" style="width:100%" class="table table-hover overflow: scroll;">
            <thead>
                <tr class="tr">
            ...
            <tbody>
                <?php $i=1; foreach ($superAdministrators as $superAdministrator): ?>
                <tr>
                    <td class="info"><?= $i ?></td>
                    <td><?= h($superAdministrator->user->username) ?></td>
                    <td><?= h($superAdministrator->name.' '.$superAdministrator->lastname) ?></td>
                    <td>
                        <?php
                            if ($superAdministrator->status==1) {
                                echo '<b style="color:#00ff00;">Activo</b><br>';
                            }
                            if ($superAdministrator->status==0) {
                                echo '<b style="color:red;">Inactivo</b><br>';
                            }
                            echo '<b>Creado: </b>'.date("d", strtotime($superAdministrator->created))."/".date("m", strtotime($superAdministrator->created))."/".date("Y", strtotime($superAdministrator->created)).'<br>';
                            echo '<b>Modificado: </b>'.date("d", strtotime($superAdministrator->modified))."/".date("m", strtotime($superAdministrator->modified))."/".date("Y", strtotime($superAdministrator->modified)).'<br>';
                        ?>
                    </td>
                    <td>
                        <button class="btn btn-block"  style="background-color: #1e9b62" onClick="open_edit(<?= $superAdministrator->id_super_administrator ?>);">Modificar</button>
                        <br>
                        <form method="get" accept-charset="utf-8" action="<?= Router::url(['action' =>'recoverypass', $superAdministrator->user->id_user], true)?>" id="<?= 'Recover'.$superAdministrator->id_super_administrator ?>">
                            <button type="button" class="btn btn-info btn-block" onClick="open_recu(<?= "'".'Recover'.$superAdministrator->id_super_administrator."'" ?>)"><span>Recuperar contraseña</span></button>
                        </form>
                    </td>
                </tr>
                <?php $i++; endforeach; ?>
    ...
    <div class="modal-dialog modal-lg" style="max-width: 500px !important;" role="document">
        <div class="modal-content" id="DocumentPrint">
            <div class="modal-header" style="background: #0d499d; color:white;">
                <h5 class="modal-title" id="TitleModalAdd">Nuevo Super Administrador</h5>
            ...
            <?= $this->Form->create(null, ["url"=>["action"=>"addusr"], "name"=>"AddUSR"]) ?>
                <div class="modal-body" id="BodyModalAdd">
                    <fieldset>
                        <input type="hidden" name="credential_id" id="credential-id" value="1">
                        <?php
...
<div style="display: none;" class="modal bd-example-modal-lg" id="modal_edit" tabindex="-1" role="dialog">
    <div class="modal-dialog modal-lg" style="max-width: 500px !important;" role="document">
        <div class="modal-content" id="DocumentPrint">
            <div class="modal-header" style="background: #0d499d; color:white;">
                <h5 class="modal-title" id="TitleModalEdit">Modificar Super Administrador</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <?= $this->Form->create(null, ["url"=>["action"=>"editusr"], "name"=>"EditUSR"]) ?>
                <div class="modal-body" id="BodyModalEdit">
                    <fieldset>
                        <input type="hidden" name="id_super_administrator" id="id-super-administrator" value="">
                        <?php
...
<script type="text/javascript">
    ...
    function closesave(){ 
        if(document.getElementById("name").value=="" || document.getElementById("lastname").value=="" || document.getElementById("email").value==""){
            ...
            swal({
                    title: "Agregar",
                    text: "¿Esta seguro de agregar este nuevo super administrador?!",   
                    type: "success",
    ...
    function open_edit(ide){
        $.ajax({
            url: route+'SuperAdministrators/locateuser',
            type:'POST',
            ...
            success: function(result){
                ...
                //console.log(output);
                document.getElementById("id-super-administrator").value = output.id_super_administrator;
    ...
    function closeedit(){ 
        if(document.getElementById("name-e").value=="" || document.getElementById("lastname-e").value=="" || document.getElementById("email-e").value==""){
            ...
            swal({
                    title: "Modificar",
                    text: "¿Esta seguro de modificar este super administrador?!",   
                    type: "success",   
                    showCancelButton: true,   
                    confirmButtonColor: "#16BA0A",   
