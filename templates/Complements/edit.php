 ...
 * @var string[]|\Cake\Collection\CollectionInterface $complementXmls
 */
?>
<div class="row">
    <aside class="column">
        <div class="side-nav">
            ...
            <?= $this->Form->postLink(
                ...
                ['confirm' => __('Are you sure you want to delete # {0}?', $complement->id_complement), 'class' => 'side-nav-item']
            ) ?>
            <?= $this->Html->link(__('List Complements'), ['action' => 'index'], ['class' => 'side-nav-item']) ?>
        </div>
    </aside>
    <div class="column column-80">
        <div class="complements form content">
            <?= $this->Form->create($complement) ?>
            <fieldset>
                <legend><?= __('Edit Complement') ?></legend>
                <?php
                    echo $this->Form->control('complement_xml_id', ['options' => $complementXmls, 'empty' => true]);
