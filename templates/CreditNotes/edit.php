 ...
 * @var string[]|\Cake\Collection\CollectionInterface $creditNoteXmls
 */
?>
<div class="row">
    <aside class="column">
        <div class="side-nav">
            ...
            <?= $this->Form->postLink(
                ...
                ['confirm' => __('Are you sure you want to delete # {0}?', $creditNote->id_credit_note), 'class' => 'side-nav-item']
            ) ?>
            <?= $this->Html->link(__('List Credit Notes'), ['action' => 'index'], ['class' => 'side-nav-item']) ?>
        </div>
    </aside>
    <div class="column column-80">
        <div class="creditNotes form content">
            <?= $this->Form->create($creditNote) ?>
            <fieldset>
                <legend><?= __('Edit Credit Note') ?></legend>
                <?php
                    echo $this->Form->control('credit_note_xml_id', ['options' => $creditNoteXmls, 'empty' => true]);
                    ...
                    echo $this->Form->control('metodo_pago');
                    echo $this->Form->control('condiciones_pago');
